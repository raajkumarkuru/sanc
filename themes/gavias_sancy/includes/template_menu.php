...
function gavias_sancy_preprocess_menu__main(&$variables) {
  $variables['attributes']['class'][] = 'clearfix';
  $language =  \Drupal::languageManager()->getCurrentLanguage()->getID();
  $languagesAll =  \Drupal::languageManager()->getLanguages();
  foreach ($variables['items'] as &$item) {
   $menu_link_attributes = _gavias_sancy_attributes_get_attributes($item['original_link']);
    if (count($menu_link_attributes)) {
      ...
      if(count($languagesAll) > 1){
        ...
        if(isset($attributes['gva_block_' . $language]) && $attributes['gva_layout']=='menu-block'){
          $item['gva_block_content'] = gavias_sancy_render_block($attributes['gva_block_' . $language]);
        }
      }else{
        ...
        if(isset($attributes['gva_block']) && $attributes['gva_layout']=='menu-block'){
          $item['gva_block_content'] = gavias_sancy_render_block($attributes['gva_block']);
        }
      }
    }
  }
...
function _gavias_sancy_attributes_get_attributes(MenuLinkInterface $menu_link_content_plugin) {
  $attributes = [];
  try {
