<?php
function gavias_sancy_font_size(){
    $size = array();
    $size[''] = '-- Default --';
    for ($i=12; $i <= 50 ; $i = $i+1) { 
        $size[$i] = $i;
...
function gavias_sancy_render_option_font(){
    $fonts = gavias_sancy_fonts();
    $output = '';
...
function gavias_sancy_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function gavias_sancy_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(gavias_sancy_fonts())) > 17){
...
function gavias_sancy_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode($json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= gavias_sancy_typography_enqueue_google_font($customize['font_family_primary']);
    }
    
    return $links_fonts;
}

function gavias_sancy_options_patterns(){
    $output = '';
    $file_path = drupal_get_path('theme', 'gavias_sancy');
